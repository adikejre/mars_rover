...
class MarsRoverEnv(gym.Env):
    def __init__(self, grid_size = dem_data_subset_cleaned.shape ,start = (50, 50), goal = (149, 349), kd=1.0, kh=30.0, kr=15.0):
...
class SaveOnBestTrainingRewardCallback(BaseCallback):
    def __init__(self, eval_freq: int, log_path: str, verbose=1):
        ...
        self.save_path = os.path.join(log_path, 'best_model_test')
                ...
                if mean_reward > self.best_mean_reward:
                    ...
                    if self.verbose > 0:
                        ...
                        with open(file, 'w') as filetowrite:
                            filetowrite.write(f"Saving new best model to {self.save_path} with mean reward {self.best_mean_reward}")
                            # filetowrite.write(str(ep_info_buffer))
        return True
...
model = PPO('MlpPolicy', env, verbose=1, tensorboard_log="ppo_logs2")
