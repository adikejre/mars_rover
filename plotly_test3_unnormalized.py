...
# fig = go.Figure(data=[go.Surface(z=dem_data_subset, x=x_coords_subset, y=y_coords_subset, colorscale='Viridis')])
...
# Create a DataFrame
terrain_data = pd.DataFrame({'x': x_flat, 'y': y_flat, 'z': z_flat})

# Save to CSV
# terrain_data.to_csv('terrain_data.csv', index=False)
